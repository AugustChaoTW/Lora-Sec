...
adding $(\eps=1.0)$-differential privacy (DP) noise further suppresses
reconstruction quality below the $0.4$ recognizability threshold.
...
\begin{table}[tb]
  ...
  \begin{tabular}{@{}lcccc@{}}
    ...
    DP ($\eps=1.0$) & \multicolumn{4}{c}{\emph{(to be measured)}} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\Cref{tab:inversionnet} (row 2) shows DP results (to be measured).
Without DP, InversionNet max SSIM $= 0.575$ (measured).
DP ($\eps=1.0$) is expected to reduce max SSIM below $0.4$.
...
point.
It is expected to provide max SSIM $< 0.4$ against InversionNet
(DP accuracy tradeoff to be measured).
...
$(\eps=1.0)$-DP Gaussian noise is expected to suppress reconstruction
below the $0.4$ threshold (DP experiment pending).
