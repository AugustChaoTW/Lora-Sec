...
//
// In the Sybil Flooding attack (E7), all data is routed to attacker 0xCC
...
//
// The gateway itself is honest and cannot detect the Sybil Flooding:
...
void send_hello() {
  ...
  if (pkt_recv == 0 && hello_count > 3) {
    Serial.println("[WARN] No packets received after 3+ Hellos — Sybil Flooding likely active");
...
void setup() {
  Serial.begin(115200);
  delay(500);
  Serial.println("\n=== E9 Node B (GATEWAY — honest, SECURITY_MODE=1) ===");
  Serial.println("[INFO] Gateway broadcasts Hello. Insider attacker (0xCC) races with own Hello.");
  Serial.println("[INFO] All honest nodes route to 0xCC (Sybil Flooding). PDR drops to 0%.");
  Serial.println("[INFO] This gateway cannot detect the Sybil Flooding — it just observes silence.");
