...
//
// In the Duty Cycle Flood attack (E7), all data is routed to attacker 0xCC
...
//
// The gateway itself is honest and cannot detect the Duty Cycle Flood:
...
void send_hello() {
  ...
  if (pkt_recv == 0 && hello_count > 3) {
    Serial.println("[WARN] No packets received after 3+ Hellos — Duty Cycle Flood likely active");
...
void setup() {
  Serial.begin(115200);
  delay(500);
  Serial.println("\n=== E10 Node B (GATEWAY — honest, SECURITY_MODE=1) ===");
  Serial.println("[INFO] Gateway broadcasts Hello. Insider attacker (0xCC) races with own Hello.");
  Serial.println("[INFO] All honest nodes route to 0xCC (Duty Cycle Flood). PDR drops to 0%.");
  Serial.println("[INFO] This gateway cannot detect the Duty Cycle Flood — it just observes silence.");
