...
one of the first studies combining Tamarin machine-checked proofs with
...
MetricVersion counter, requiring no PKI infrastructure.
The finding reclassifies the attack as a confirmed traffic black hole
rather than an adversarial relay—a distinction with direct implications
for intrusion detection: gateway-centric monitoring observes only
silence, so effective detection requires relay-side observation.
...
We note that these detection strategies are derived from
protocol-layer analysis; quantitative evaluation of their
false-positive rates and energy overhead on constrained hardware
remains outside the scope of this study and is left for future work.
...
Meshtastic~v2.6+ avoids the dst-field collapse entirely:
its \texttt{MeshPacket} header carries a separate \texttt{next\_hop}
field alongside the final \texttt{to} field.
Our Tamarin analysis of the Meshtastic routing model shows that
this design eliminates the silent-black-hole category but
introduces a \emph{different} symbolic-layer divergence:
an unauthenticated \texttt{next\_hop} lets an adversary become
an active relay (intercept, inspect, selectively drop) rather
than an absent destination.
Lemmas L1--L3 are falsified, L4 identifies an admin-command
replay vulnerability, and the appropriate IDS architecture
shifts from sender-centric anomaly detection to relay-path
authentication—a distinct design space from the LoRaMesher case.
...
\begin{itemize}
  ...
  \item \textbf{Layer~3 --- Resource exhaustion}: Three DoS vectors
    ...
    (no HMAC on \texttt{AckMsg}), Sybil table exhaustion
...
\begin{itemize}
  ...
  \item \textbf{Hardware experiment logs}: Serial logs for E5b--E15
    in \texttt{hardware\_experiments/}; E7--E15 characterize open
    threat vectors and are designated for a companion study.
\end{itemize}
