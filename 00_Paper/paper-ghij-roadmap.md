...
> **Open LoRa Mesh Dataset (OLMD)** 是整個 G–K 計畫的引力中心：  
> B–F 從 OLMD 獲得 in-the-wild validation → 可視性提升；  
> G–J 的實驗數據來自 OLMD → 每篇都有真實世界背書；  
> K 是 OLMD 的 dataset paper，越早上線越好。  
...
## 二、三層 GenAI 架構 + OLMD 基礎設施（LeCun 第 64 輪）

```
┌─────────────────────────────────────────────────────────────┐
│  OLMD — Open LoRa Mesh Dataset  (K-infra，現在啟動)          │
...
### Paper K — Open LoRa Mesh Dataset (OLMD): Long-Term Observatory for AI-Assisted LoRa Mesh Security
...
| **OLMD（目標）** | **多地點 + 長期季節性 + security 標注 + AI-ready schema** |

#### OLMD Dataset Schema（每筆記錄）
...
- 這讓 Paper F 的抽象理論在 OLMD 中有具體 enforcement 機制
...
| K4 | OLMD 季節性 RF 數據能訓練 Paper J §7.5 的 long-term RF prediction model | LSTM/Transformer cross-region 訓練 + 驗證 |
...
Week 1 (現在)
  ...
  □ 定義 OLMD schema v0.1（基於本文件）
  □ Meshtastic Discord 發公告：「Open LoRa Observatory beta」
...
Week 2–4
  ...
  □ OLMD Tier 0 公開：https://olmd.io（aggregate stats only）

Month 2–5
  □ 擴展至 50+ 節點（4 個地區）
  □ 每月 Discord progress update
  □ Paper G/H/I 論文標注「data from OLMD」

Month 6
  □ 有 6 個月 trace → K-paper 開始寫
  □ OLMD v1.0 Tier 1 開放（arxiv 同步）
```

#### 核心貢獻（Paper K 論文）
1. **OLMD 數據集**：多地點 × 長期 × 四層 schema × 安全標注（主角）
...
4. **Five use cases**：B/C/E/F/J 在 OLMD 上的 in-the-wild validation
...
Foundation (#1–29)
    │
    ▼
Paper B ✓ (AdHocNet)                ◄── OLMD UC1: B attack in-the-wild
    │  dst=next_hop→silent black hole; HMAC patch
    ├──► Paper C (TIFS)              ◄── OLMD UC2: cross-protocol event distribution
    │       cross-protocol taxonomy; #53 draft pending
    ├──► Paper D (IoTJ)
    │       selective encryption; #67 E21 open (P3)
    ├──► Paper E (IoTJ/FGCS)        ◄── OLMD UC3: embedding privacy in-the-wild
    │       split inference + MIA; #72 E20 INA219 open
    └──► Paper F ✓ (S&P/USENIX)    ◄── OLMD UC4: PCSS Φ(P,E) live enforcement
             PCSS theory
             │
    ┌────────────────────────────────────────────────────────────────┐
    │                                                                │
    ...
    │  │                                                          │ │
    │  │  Paper G (ToSN/TDSC)          ← OLMD security event log │ │
    │  │  Cloud LLM: evolvable threat model + fuzzing             │ │
    │  │  Depends: B, C, F + [1,2,9]                             │ │
    │  │      │                                                   │ │
    │  │      ├──► Paper H (TIFS/JSAC) ← OLMD RF fingerprint     │ │
    │  │      │    Node: mesh-aware RFFI attestation              │ │
    │  │      │    Depends: B, E + [3,4,5]                       │ │
    │  │      │                                                   │ │
    │  │      └──► Paper I (TMC/IoTJ)  ← OLMD multi-modal trace  │ │
    │  │           Gateway: cross-layer quant-xformer             │ │
    │  │           Depends: B, C, G, H + [6,7]                   │ │
    │  │               │                                          │ │
    │  │               ▼                                          │ │
    │  │           Paper J (SenSys/INFOCOM) ← OLMD UC5 seasonal  │ │
    │  │           System: scaling + real deployment              │ │
    │  │           Depends: G+H+I + [8,10]                       │ │
    │  │           90-day trace → 回饋 OLMD v1.1                  │ │
    │  └──────────────────────────────────────────────────────────┘ │
    │                    │                                           │
    └────────────────────┼───────────────────────────────────────────┘
                         ▼
    Paper K (NeurIPS D&B / SIGCOMM / TPDS)
    Dataset paper: OLMD + Observatory + Privacy + Ethics
...
§1  Introduction (2p)
    ...
    §1.4  Contribution map (G/H/I/J/K + OLMD)
...
§3  System Architecture (2p)
    §3.1  OLMD as the data foundation (Fig: K-infra flow)
    §3.2  Three-layer AI stack (Fig: node → gateway → cloud)
    §3.3  Packet format evolution (B 142B → H 206B attestation)
    §3.4  PCSS grounding (Paper F): Φ(P,E) as privacy enforcement in OLMD
...
§4  Layer 1: Cloud — Evolvable Threat Model (Paper G) (3p)
    ...
    §4.4  OLMD data use: G threat labels annotate OLMD security_event

§5  Layer 3: Node — Mesh-Aware RFFI Attestation (Paper H) (3p)
    §5.1  Multi-hop fingerprint degradation measurement (OLMD RF trace)
...
§6  Layer 2: Gateway — Cross-Layer Detection (Paper I) (3p)
    §6.1  Multi-modal feature fusion (OLMD multi-layer trace as training set)
...
§7  System Integration: Scaling & Real Deployment (Paper J) (4p)
    ...
    §7.3  90-day outdoor deployment (trace → OLMD v1.1)
...
§8  OLMD Dataset & Observatory (Paper K summary) (2p)
    ...
    §8.4  Use cases UC1–UC5: how OLMD validates B–F in the wild
...
§9  Discussion (1.5p)
    ...
    §9.4  PCSS degradation window: when does OLMD stop publishing?
...
KEY MILESTONES
 2026-05  K-infra launch: 5 nodes + OLMD schema v0.1 + Discord post #1
 2026-06  Architecture locked; OLMD Tier 0 public
 2026-07  OLMD 20+ nodes (3 cities); G+H experiments done
 2026-08  G+H submission-ready; v0.1 code release (ChirpOTLE+attestation+YAML)
 2026-09  OLMD 50+ nodes; Tier 1 open; Paper I experiments done
 2026-11  J 90-day deployment starts
 2026-12  I+J submission ready; OLMD v1.0 with 6-month trace
 2027-01  K-paper writing starts (6-month trace in hand)
 2027-02  Meshtastic RFC draft
 2027-03  K-paper submission
 2027-04  IETF LPWAN working group contact; OLMD public beta
```
...
| 項目 | 說明 | 期限 |
|------|------|------|
| OLMD 命名確認 | "OLMD" 不夠易記，需要類 ImageNet/COCO 的名字 | 本週 |
| OLMD schema v0.1 | 基於本文件 YAML spec，建 GitHub repo | 本週 |
...
| 項目 | 說明 | 影響 |
|------|------|------|
| OLMD 命名 domain | olmd.io 或 [name].io 域名註冊 | K-infra 品牌 |
...
| IRB 申請 | OLMD Tier 2 開放前必須通過 | K-paper 合規性 |

### 長期（6–12 個月）

| 項目 | 說明 | 影響 |
|------|------|------|
| 90-day 部署場地 | 戶外有遮蔽環境（類 Abrardo aqueduct） | Paper J J3 + OLMD 季節性數據 |
| Paper C #53 draft | 唯一缺 draft 的現有論文 | 不影響 G–K 但應優先關閉 |
| Industry partner | Semtech / RAK Wireless / TTN 接洽 | OLMD 節點擴展 + K paper impact |
