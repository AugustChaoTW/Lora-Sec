// E8: ACK Spoofing Attack Demo
//
// Vulnerability: AckMsg carries no HMAC. Any node can forge an ACK
// claiming to be the gateway (0xBB). The HMAC patch (E6/E7) authenticates
// Hello and Data messages but never extended to ACK.
//
// Attack topology:
//   A(0xAA) → C(0xCC, attacker-as-relay) → B(0xBB, gateway)
//
// Attack sequence:
//   1. C broadcasts Hello(metric=1, HMAC=valid) — honest relay role
//   2. A routes via C (C is only path to B in this topology)
//   3. A sends DATA to C (next hop)
//   4. C receives DATA, immediately sends forged ACK(src=0xBB, dst=0xAA, pkt_id=N)
//   5. A marks packet "delivered" → sender-side PDR = 100%
//   6. C DROPS the data — B receives 0 packets → gateway PDR = 0%
//
// Finding:
//   HMAC patch covers MSG_HELLO and MSG_DATA but NOT MSG_ACK.
//   "Authenticated routing does not imply authenticated delivery confirmation."
//   Gap: AckMsg should include HMAC(src, dst, pkt_id, NETWORK_PSK).
//
// Proposed minimal fix (not deployed here — shown as comment):
//   Add hmac[HMAC_TAG_LEN] to AckMsg, call sign_ack() before sending.
//
...
// ACK has NO HMAC — this is the vulnerability
struct AckMsg {
  uint8_t  type;
  uint8_t  src;      // spoofed to 0xBB by attacker
  uint8_t  dst;
  uint16_t pkt_id;
} __attribute__((packed));
