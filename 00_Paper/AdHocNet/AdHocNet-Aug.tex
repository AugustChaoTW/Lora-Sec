...
\begin{abstract}
Route injection in LoRaMesher—an open-source distance-vector mesh
routing stack for ESP32-class LoRa nodes—creates a
...
77.4\% under attack on tree and grid), is invisible to both
Tamarin formal models and ns-3 packet-level simulation, and only
becomes visible through physical hardware serial-log inspection.

LoRaMesher's control plane carries no cryptographic authentication,
enabling this attack by any adversary with a commodity \$20 LoRa
transceiver in networks deployed across precision agriculture,
...
\begin{itemize}
  ...
  \item \textbf{Layer~4 --- Behavioral anomaly detection}: No
    ...
    ESP32-S3 without floating-point operations (measured KLD score
    754 for flood-attack vs.\ threshold 200, 2 alerts in 120~s); and
    (iii)~on-device autoencoder (E15, 169~parameters, 676~bytes RAM,
    TPR=100\%, FPR=5\%, inference 6--7~$\mu$s on ESP32-S3 at 240~MHz),
    extending~\cite{springer2026-blockchain-ids}
    to constrained hardware.
    \begin{table}[h]
      \centering
      \caption{On-device IDS approaches firmware-validated on EoRa-PI
               (ESP32-S3 + SX1262, 120~s runs)}
      \label{tab:ids-comparison}
      \setlength{\tabcolsep}{4pt}
      \begin{tabular}{llccc}
        \toprule
        \textbf{Method} & \textbf{Exp} & \textbf{Inference} & \textbf{TPR} & \textbf{FPR} \\
        \midrule
        FSM Guardian      & E12 & rule-based    & —      & — \\
        KLD + HD IDS      & E13 & integer ops   & —      & — \\
        Autoencoder (AE)  & E15 & \textbf{6--7~$\mu$s} & 100\%  & 5\% \\
        \bottomrule
      \end{tabular}
    \end{table}
  ...
  \item \textbf{Layer~0 --- Physical-layer authentication}: The
    ...
    can estimate attacker position via weighted least-squares
    multilateration over five observer nodes—without SDR
    infrastructure.
    Hardware validation produced 205 stable position estimates in
    120~s (all converging to $<$1~cm error at bench scale);
    path-loss calibration is required for meter-scale accuracy
    at LoRa operating distances.
