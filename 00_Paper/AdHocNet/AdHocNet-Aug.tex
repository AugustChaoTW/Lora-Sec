...
Our three-tier validation reveals that route injection in
LoRaMesher creates a \emph{silent traffic black hole}—not an
adversarial relay. The attack mechanism is invisible to both the
Tamarin symbolic model (which abstracts destination as an agent
identity) and the ns-3 simulator (which routes by gateway address):
only physical board serial logs reveal that the firmware stores the
attacker's next-hop address in the \texttt{dst} field, ensuring that
the gateway never receives the diverted packets.
This changes how the attack must be characterized for deployment
advisories and intrusion detection design—an IDS monitoring for
anomalous traffic \emph{at the gateway} would observe silence, not
an attack signature.
...
\begin{enumerate}
  \item \textbf{Cross-Layer Routing Semantics Gap.}
    We identify that route injection in LoRaMesher creates a
    \emph{silent traffic black hole}—not an adversarial relay—because
    the deployed firmware encodes next-hop forwarding state in the
    destination field rather than the final gateway identity assumed
    by symbolic and packet-level analyses.
    This cross-layer semantic mismatch: (i)~is invisible to both the
    Tamarin symbolic model and the ns-3 packet-level simulator;
    (ii)~explains the topology-dependent PDR gradient
    (0\% on linear; 22.6\% degradation on tree and grid);
    (iii)~is observable only through physical hardware serial-log
    analysis; and (iv)~changes the threat model from a potential
    man-in-the-middle to a confirmed silent black hole, with direct
  ...
  \item \textbf{Machine-Checked Vulnerability Proofs and Security
    Guarantees.}
    We construct Tamarin Prover models of LoRaMesher's control plane
    and obtain machine-checked counterexample traces for two attack
    classes—route injection and rekey-epoch replay—that falsify
    four of five baseline security lemmas.
    We then formally verify a corrective patch satisfying all eight
    security goals (Section~\ref{sec:formal}).

  \item \textbf{Formally Verified Lightweight Patch.}
    We design a minimal patch for ESP32-class microcontrollers—
    HMAC-SHA256 per-message authentication and a per-destination
    NVS-persistent MetricVersion counter—that closes both attack
    classes with negligible overhead and no PKI dependency.
    Tamarin verifies all security goals (Section~\ref{sec:patch}).

  \item \textbf{Multi-Tier Validation Across Simulation and Hardware.}
    We validate across ns-3 simulation runs spanning three topologies
    and scales up to 49~nodes (0\% residual PDR degradation under
    full patching) and three hardware experiments on six physical
    EoRa-PI boards confirming attack, defense, and reboot-replay
    behavior on deployed ESP32-S3 firmware
    (Section~\ref{sec:eval}).

  \item \textbf{Open, Reproducible Artifact Suite.}
    All Tamarin models, ns-3 simulation code, patched firmware, and
    hardware serial logs are released under MIT license
...
Resource-constrained mesh stacks routinely overload
packet-header fields to minimize airtime and memory overhead:
a single field serves both as a routing-metadata carrier
and as an application-layer endpoint identifier, collapsing
two distinct semantic roles into one byte sequence.
This design choice is rational under severe payload and RAM
budgets—LoRaMesher eliminates a separate next-hop header and
saves 6 bytes per packet—but it means that protocol-layer
models, which follow the conventional abstraction that
destination fields denote semantic endpoints, may
\emph{systematically diverge} from deployed forwarding
semantics across the entire class of constrained mesh stacks.
The mismatch is therefore not a firmware bug specific to
LoRaMesher; it is a structural consequence of constrained
mesh design that protocol-layer analyses inherit by default.

The pattern is not unique to LoRaMesher.
RPL~\cite{ietf-rfc7416} encodes preferred parent identity in the
DODAG parent set; a formal analysis that abstracts ``preferred
parent'' as an endpoint identity would miss the next-hop
binding semantics.
BATMAN-adv uses MAC-layer originator messages whose forwarder
address is distinct from the originator address in a way that
symbolic models typically collapse.
Any constrained mesh protocol that inlines routing metadata
(next-hop, interface index, forwarder identity) into fields that
symbolic models treat as semantic endpoints is susceptible to
the same modeling gap.
...
\begin{itemize}
  ...
  \item \textbf{Layer~2 --- Topology deception}: The HMAC +
    ...
    forwarding failure.
    Detecting topology-deception attacks requires hop-count
    consistency verification or path-diversity watchdog mechanisms,
  ...
  \item \textbf{Layer~3 --- Resource exhaustion}: The patch does
    ...
    application-layer acknowledgments; (ii)~Sybil address flooding,
    which can exhaust the routing table given LoRaMesher's
    \texttt{uint16\_t} address space; and (iii)~Hello flooding
    against AS923's 1\% duty-cycle budget—an attacker can force
    honest nodes to consume airtime responding to
    adversarially-timed control messages, causing data-plane
    starvation without violating any cryptographic protection.
  ...
  \item \textbf{Layer~4 --- Behavioral anomaly detection}: No
    ...
    KLD-based Hello-interval analysis or FSM state-transition
    monitoring on a dedicated guardian node can complement the
    cryptographic patch, but requires calibration on per-deployment
    baseline traffic and is deferred to future work.
  ...
  \item \textbf{Layer~0 --- Physical-layer authentication}: The
    ...
    characteristics~\cite{ieee2025-lora-adversarial} can
    complement HMAC by providing a protocol-independent
    authentication signal, but requires SDR infrastructure not
