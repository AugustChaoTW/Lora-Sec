...
We show that successful route injection produces a \emph{silent
...
packet-level simulation; it becomes visible only through physical
hardware serial-log inspection.

We address this gap with a three-tier methodology: Tamarin
machine-checked proofs confirming four attack lemmas on the
unpatched protocol, ns-3 simulation across 135 scenarios and
up to 49 nodes, and validation on six physical EoRa-PI boards.
We design a lightweight patch—HMAC-SHA256 per-message
authentication combined with a per-destination MetricVersion
counter—that requires no PKI infrastructure.
The patched protocol achieves 0\% residual PDR degradation under
...
ARIADNE~\cite{hu2002-ariadne}) assume either PKI infrastructure or
time-based one-way function chains (TESLA) requiring clock synchronisation
infeasible under LoRa's duty-cycle and sleep-cycle constraints.
TinyECC~\cite{liu2008-tinyecc} and similar lightweight
...
The counter is declared \texttt{uint16\_t}; at a 10\,s Hello interval, exhaustion requires $65{,}535 \times 10\,\text{s} \approx 7.6$~days—sufficient for most deployments, though operators with multi-week unattended deployments should monitor for wrap-around or plan a session rekey within that window.
...
A preliminary informal analysis of the Meshtastic routing model suggests that
...
replay vulnerability (these observations are based on manual protocol inspection;
a machine-checked Tamarin model of Meshtastic routing is deferred to future work),
and the appropriate IDS architecture
