// E7: Black Hole Attack Demo
// Extends E6 to demonstrate that HMAC patch alone is insufficient
// against an insider adversary holding the network PSK.
//
// Topology: A(0xAA) → R1(0x11) → R2(0x22) → R3(0x33) → B(0xBB)
//           Attacker C(0xCC) — holds valid PSK, signs its own Hellos
//
// Attack: C broadcasts Hello(metric=0) SIGNED with valid HMAC.
//   SECURITY_MODE=1 (patched) nodes accept the Hello (HMAC passes).
//   All nodes route data to 0xCC. C silently drops everything.
//   Gateway receives 0 packets despite HMAC patch being active.
//
// Finding: Authentication ≠ Availability.
//          HMAC proves origin, not forwarding behavior.
//
...
// Network PSK — same key provisioned to all nodes including attacker
// In a real deployment this represents a compromised/insider node
