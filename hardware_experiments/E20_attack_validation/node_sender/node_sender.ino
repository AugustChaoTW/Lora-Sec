...
// E20 Packet layout:
//   All frags:   | magic(2B:E1,AA) | frag_id(2B) | total_frags(2B) | flags(1B) | payload(N) | hmac8(8B) |
//   frag_id=0:   payload[0..3] = image_seq (big-endian 4B) for F3 replay resistance
//
// Sender uses magic 0xE1,0xAA; relay translates to 0xE1,0x9A when forwarding.
// Receiver only accepts 0xE1,0x9A — ignores direct sender broadcasts.
// This ensures receiver sees only relay-controlled packets (drop/flip/replay testable).
#define E19_MAGIC_0   0xE1
#define E19_MAGIC_1   0xAA   // sender→relay magic
#define FRAG_HEADER_SIZE  7   // magic(2)+frag_id(2)+total_frags(2)+flags(1)
#define HMAC_TRUNC_SIZE   8   // 64-bit truncated HMAC
#define SEQ_PREFIX_SIZE   4   // image_seq prefix in frag_id=0 payload (F3 fix)
...
void send_image() {
  ...
  // E20: chain init includes image_seq (F3 replay resistance fix)
  // sess_data = <image_seq(4B big-endian) | total_frags(2B big-endian)>
  // Receiver reads image_seq from frag_id=0 payload[0..3].
  {
    uint8_t sess_data[6];
    sess_data[0] = (image_seq >> 24) & 0xFF; sess_data[1] = (image_seq >> 16) & 0xFF;
    sess_data[2] = (image_seq >>  8) & 0xFF; sess_data[3] =  image_seq         & 0xFF;
  ...
  for (uint16_t frag = 0; frag < total_frags; frag++) {
    ...
    // E20: frag_id=0 prepends 4B image_seq to payload for receiver to reconstruct h0 (F3)
    pkt[0] = E19_MAGIC_0; pkt[1] = E19_MAGIC_1;
    pkt[2] = (frag >> 8) & 0xFF; pkt[3] = frag & 0xFF;
    pkt[4] = (total_frags >> 8) & 0xFF; pkt[5] = total_frags & 0xFF;
    pkt[6] = flags;
    if (frag == 0) {
      pkt[FRAG_HEADER_SIZE + 0] = (image_seq >> 24) & 0xFF;
      pkt[FRAG_HEADER_SIZE + 1] = (image_seq >> 16) & 0xFF;
      pkt[FRAG_HEADER_SIZE + 2] = (image_seq >>  8) & 0xFF;
      pkt[FRAG_HEADER_SIZE + 3] =  image_seq         & 0xFF;
      memcpy(pkt + FRAG_HEADER_SIZE + SEQ_PREFIX_SIZE, img + offset, plen - SEQ_PREFIX_SIZE);
      plen = plen;  // total plen unchanged; SEQ_PREFIX_SIZE bytes replace start of payload
    } else {
      memcpy(pkt + FRAG_HEADER_SIZE, img + offset, plen);
    }

    // FC-HMAC over actual transmitted payload (pkt buf), not raw img buf.
    // For frag_id=0 the payload starts with 4B img_seq prefix — must hash exactly
    // what the receiver will see, otherwise chain breaks at frag_id=0.
    uint8_t hmac8[8];
    fc_hmac(frag, flags, pkt + FRAG_HEADER_SIZE, plen, prev_hmac, hmac8);
    ...
    delay(2500); // inter-frame gap: relay TX takes ~1.7s; 2.5s ensures relay finishes TX and re-arms RX before next frag arrives
  }
