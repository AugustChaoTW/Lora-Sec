...
many such deployments is LoRaMesher~\cite{sole2022-loramesher,sole2024-loramesher-dv},
...
\begin{table}[t]
  ...
  \begin{tabular}{lcc}
    ...
    LoRa airtime (Hello) & 124~ms (SF9)  & 165~ms patched (+33\%) \\
    Duty cycle (Hello) & 1.24\%/10~s    & 1.65\%/10~s (+0.41 pp) \\
...
The patch appends an 8-byte truncated HMAC tag to each
control message (a deliberate design choice; 8~bytes provides
64-bit collision resistance, sufficient against Dolev-Yao adversaries
that cannot break SHA-256). Using the Semtech LoRa ToA formula at
SF9 / BW=125~kHz / CR=4/5 / explicit header / CRC:
a Hello message grows from 6~bytes (baseline) to 14~bytes (patched),
increasing time-on-air from $\approx$124~ms to $\approx$165~ms
(+33\%). The per-node duty-cycle increase for Hello traffic is
$(165-124)/10{,}000 = 0.41$~percentage points. For data packets
(13~bytes $\to$ 21~bytes), the airtime increase is $\approx$12\%.
In the AS923 band (used in this study), the 1\% per-channel
duty-cycle ceiling applies; the additional $\approx$0.41~pp overhead
...
reduce HMAC to 4~bytes (32-bit tag), halving the airtime overhead
...
can inject one stale route entry. This one-reboot attack window
is a known limitation and motivates persistent-counter storage
(e.g., NVS flash) as future work.
...
AS923/923~MHz, 10~dBm) as a \emph{proof-of-concept} demonstration.
This experiment confirms that the attack and defense behave as
predicted on production hardware; deployment-scale coverage
(multi-hop topologies, 6–9 nodes) is provided by the ns-3 results
in Table~\ref{tab:attacks}.
