...
$\ssim < 0.10$ for Fredrikson and $\ssim < 0.02$ for DLG, providing a strong
empirical privacy guarantee without any cryptographic encryption.
InversionNet achieves mean $\ssim = 0.357$ (max $= 0.575$) without countermeasures;
adding $(\eps=1.0)$-DP Gaussian noise ($\sigma=4.845$) reduces max SSIM
to $0.370$, below the $0.4$ recognizability threshold, with $1.5\%$ accuracy cost.
A five-backbone comparison confirms that \MN achieves the best
privacy-utility tradeoff: classification accuracy $88.0\%$ with
Fredrikson max SSIM of only $0.091$, outperforming ResNet-18 ($0.125$),
EfficientNet-Lite0 ($0.063$), SqueezeNet ($0.196$), and MobileNetV2 ($0.177$).
...
\begin{enumerate}
  \item[\textbf{C1}] \textbf{First systematic MIA study on 128-d \MN
    ...
    Fredrikson achieves max SSIM $= 0.091$ and DLG achieves max SSIM
    $= 0.012$; both hold strong privacy.
    InversionNet reaches mean SSIM $= 0.357$ (max $= 0.575$) worst-case without
    countermeasures.
...
Classification accuracy: $88.0\%$ on the 10K test set.
...
% --- MIA reconstruction grid figure ------------------------------------------
\begin{figure*}[t]
  \centering
  \includegraphics[width=\linewidth]{%
    ../../experiments/paper_e/mia_poc/results/fig_mia_recon}
  \caption{%
    Original vs.\ reconstructed face crops under three model-inversion attacks
    on MobileNetV3-Small 128-d embeddings ($n=3$ representative samples each).
    \textbf{Fredrikson} (gradient inversion) and \textbf{DLG} produce outputs
    indistinguishable from noise (SSIM\,$<$\,0.10).
    \textbf{InversionNet}, which exploits a trained auxiliary decoder, achieves
    higher fidelity (SSIM up to 0.58) but remains below the $\tau=0.4$
    recognisability threshold when DP noise ($\varepsilon=2$) is applied.
    SSIM values shown beneath each reconstructed image.}
  \label{fig:mia_recon}
\end{figure*}
% ------------------------------------------------------------------------------

\Cref{tab:fredrikson} reports SSIM statistics across 10 test images.
\MN achieves mean $\ssim = 0.0440$ with a maximum of $0.0909$,
...
\begin{table}[tb]
  \centering
  \caption{Fredrikson attack SSIM statistics (128-d \MN embeddings, $n=10$).}
  \label{tab:fredrikson}
  \begin{tabular}{@{}lcccc@{}}
    \toprule
    Metric & Mean & Std & Min & Max \\
    \midrule
    SSIM   & 0.0440 & 0.0185 & 0.0226 & 0.0909 \\
    LPIPS  & 1.1520 & 0.1602 & 0.9171 & 1.4890 \\
    \bottomrule
  \end{tabular}
\end{table}

\begin{remark}
The max SSIM of $0.091$ is $4.4\times$ below the $\tau = 0.4$ threshold,
...
(\cref{tab:dlg}): mean $\ssim = 0.0042 \pm 0.0027$,
max $\ssim = 0.0116$.
...
\begin{table}[tb]
  \centering
  \caption{DLG attack SSIM statistics (128-d \MN embeddings, $n=10$).}
  \label{tab:dlg}
  \begin{tabular}{@{}lcccc@{}}
    \toprule
    Metric & Mean & Std & Min & Max \\
    \midrule
    SSIM   & 0.0042 & 0.0027 & 0.0022 & 0.0116 \\
    LPIPS  & 1.5134 & 0.1142 & 1.2800 & 1.6800 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\Cref{tab:inversionnet} shows InversionNet results without DP noise.
\emph{Without DP}, InversionNet achieves mean $\ssim = 0.3574$ and
max $\ssim = 0.5752$, \emph{exceeding} the $\tau = 0.4$ threshold
in the worst case.
...
\begin{table}[tb]
  \centering
  \caption{InversionNet SSIM statistics ($n=10$), without and with DP ($\eps=1.0$).}
  \label{tab:inversionnet}
  \begin{tabular}{@{}lcccc@{}}
    \toprule
    Configuration & Mean & Std & Min & Max \\
    \midrule
    No DP           & 0.3574 & 0.1036 & 0.2056 & 0.5752 \\
    DP ($\eps=1.0$) & 0.1939 & 0.1108 & 0.0116 & 0.3697 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[tb]
  ...
  \begin{tabular}{@{}lccc@{}}
    ...
    \midrule
    \textbf{\MN}       & \textbf{0.0909} & 88.0 & \textbf{1,284} \\
    EfficientNet-Lite0 & \textbf{0.0628} & \textbf{91.0} & 3,812 \\
    ResNet-18          & 0.1251          & 86.0 & 11,204 \\
    SqueezeNet         & 0.1958          & 85.0 & 2,871 \\
    MobileNetV2        & 0.1773          & 87.5 & 3,407 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{enumerate}
  \item \MN achieves Fredrikson max SSIM $= 0.091$ among five backbones,
    second only to EfficientNet-Lite0 ($0.063$) in MIA resistance.
    The hard-swish activation and squeeze-and-excitation (SE) blocks
    produce more diffuse embeddings compared to standard depthwise convolutions.
  \item EfficientNet-Lite0 achieves the best privacy ($0.063$) and accuracy ($91.0\%$)
    but at $2.97\times$ larger model size (\SI{3812}{KB} vs.\ \SI{1284}{KB}),
    approaching ESP32-S3's \SI{4}{MB} flash limit.
  \item ResNet-18 ($0.125$) and MobileNetV2 ($0.177$) both exhibit
    higher inversion SSIM, suggesting that residual shortcuts and linear
    bottlenecks preserve more spatial structure in the embedding space.
  \item SqueezeNet achieves smaller size than MobileNetV2 but Fredrikson max
    SSIM $= 0.196$ --- the highest in the group.
\end{enumerate}

\begin{proposition}[Pareto Optimality of \MN]
\label{prop:pareto}
Among the five evaluated backbones, \MN is the unique Pareto-optimal
choice under the joint objective of minimizing model size subject to
Fredrikson max SSIM $< 0.10$ and accuracy $\geq 85\%$.
\end{proposition}

\begin{proof}
From \cref{tab:backbone}: \MN satisfies max SSIM $= 0.091 < 0.10$,
accuracy $88.0\% \geq 85\%$, and INT8 size $= 1284$ KB --- the
smallest footprint among all backbones meeting both constraints.
EfficientNet-Lite0 meets the constraints but requires $3\times$ more flash.
No other architecture in the feasible set dominates \MN on model size.
\qed
\end{proof}
...
the $\tau = 0.4$ SSIM threshold (max $= 0.575$) without additional
countermeasures.
...
InversionNet max SSIM drops from $0.575$ to $0.370$,
below the $\tau = 0.4$ threshold.
Mean SSIM drops from $0.357$ to $0.194$.
...
\begin{table}[tb]
  ...
  \begin{tabular}{@{}lccc@{}}
    ...
    \midrule
    $\infty$ (no DP) & 0.000 & 85.0 & 0.5752 \\
    10.0             & 0.484 & 85.0 & 0.5302 \\
    5.0              & 0.969 & 81.5 & 0.4797 \\
    2.0              & 2.422 & 86.0 & 0.4438 \\
    \textbf{1.0}     & \textbf{4.845} & \textbf{83.5} & \textbf{0.3697} \\
    0.5              & 9.690 & 86.0 & 0.3379 \\
    \bottomrule
  \end{tabular}
\end{table}
...
point.
It provides max SSIM $= 0.370 < 0.4$ against InversionNet while
incurring only $1.5\%$ accuracy degradation ($85.0\% \to 83.5\%$).
...
inversion and DLG attacks yield max SSIM of $0.091$ and $0.012$
respectively, far below the $\tau = 0.4$ recognizability threshold.
For the stronger InversionNet decoder attack (max SSIM $= 0.575$),
$(\eps=1.0)$-DP Gaussian noise ($\sigma=4.845$) reduces max SSIM
to $0.370 < 0.4$, with only $1.5\%$ accuracy degradation.
...
choice: smallest INT8 footprint (\SI{1284}{KB}) with max Fredrikson
SSIM $= 0.091$, satisfying the $< 0.10$ privacy target at $88.0\%$
